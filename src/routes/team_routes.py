...
def generate_teams_pdf():
    ...
    try:
        ...
        # Função para sanitizar texto para uso no PDF
        def sanitize_text(text):
            if text is None:
                return ""
            # Remover caracteres não ASCII e substituir por aproximações
            text = str(text)
        ...
        # Caminho para a fonte DejaVuSans
        dejavu_sans_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "static", "fonts", "DejaVuSans.ttf")
        
        # Verificar se o arquivo da fonte existe
        if not os.path.exists(dejavu_sans_path):
            return jsonify({"message": "Arquivo de fonte DejaVuSans.ttf não encontrado."}), 500
        ...
        # Adicionar e registrar a fonte DejaVuSans
        ...
        # Usar a fonte DejaVuSans como padrão
        ...
        pdf.cell(0, 10, sanitize_text("Formação dos Times de Golfe"), align="C")
        pdf.ln(15)
        
        # Data
        pdf.set_font("helvetica", "I", 10)
        pdf.set_text_color(100, 100, 100)
        pdf.cell(0, 10, sanitize_text(f"Data: {datetime.now().strftime('%d/%m/%Y')}"), align="R")
        pdf.ln(15)
        ...
        for team in teams:
            ...
            team_title = sanitize_text(f"Time {team['team_number']} (HCP Ponderado Total: {team['total_weighted_handicap']})")
            pdf.cell(0, 10, team_title, 1, 1, "L", True)
            ...
            for player in team["players"]:
                ...
                # Sanitizar o nome do jogador para evitar caracteres problemáticos
                player_name = sanitize_text(player['name'])
                player_info = sanitize_text(f"{player_name} (HCP: {player['handicap']}, Cat: {category})")
                pdf.cell(0, 8, player_info, 1, 1, "L", True)
        ...
        pdf.set_font("helvetica", "B", 16)
        pdf.set_text_color(44, 94, 26)  # Verde escuro (#2c5e1a)
        pdf.cell(0, 10, sanitize_text("Regras Gerais"), align="C")
        pdf.ln(15)
        ...
        pdf.cell(0, 10, sanitize_text("1. Cálculo do Handicap Ponderado"), 1, 1, "L", True)
        
        pdf.set_font("helvetica", "", 10)
        pdf.set_text_color(0, 0, 0)  # Preto
        pdf.multi_cell(0, 6, sanitize_text("O handicap de cada jogador é ponderado de acordo com sua categoria para equilibrar os times:"), 0, "L")
        
        # Lista de categorias
        pdf.ln(2)
        pdf.cell(10, 6, "", 0, 0)
        pdf.cell(0, 6, sanitize_text("• Categoria A (0-10): 25% do handicap"), 0, 1)
        pdf.cell(10, 6, "", 0, 0)
        pdf.cell(0, 6, sanitize_text("• Categoria B (11-18): 20% do handicap"), 0, 1)
        pdf.cell(10, 6, "", 0, 0)
        pdf.cell(0, 6, sanitize_text("• Categoria C (19-27): 15% do handicap"), 0, 1)
        pdf.cell(10, 6, "", 0, 0)
        pdf.cell(0, 6, sanitize_text("• Categoria D (28+): 10% do handicap"), 0, 1)
        pdf.ln(2)
        
        pdf.multi_cell(0, 6, sanitize_text("Exemplo: Um jogador com handicap 8 (categoria A) terá handicap ponderado de 2 (25% de 8). Este cálculo é utilizado para garantir uma distribuição mais justa e equilibrada dos times, considerando as diferentes habilidades dos jogadores."), 0, "L")
        
        pdf.ln(2)
        pdf.multi_cell(0, 6, sanitize_text("Nota: Quando necessário formar times com diferentes números de jogadores, o jogador da categoria A com menor handicap será priorizado para ficar em times de 3 jogadores, maximizando seu impacto no jogo."), 0, "L")
        pdf.ln(5)
        
        # Regra 2
        pdf.set_font("helvetica", "B", 12)
        pdf.set_text_color(44, 94, 26)  # Verde escuro
        pdf.cell(0, 10, sanitize_text("2. Alívio de Um Taco (Não Mais Perto do Buraco)"), 1, 1, "L", True)
        
        pdf.set_font("helvetica", "", 10)
        pdf.set_text_color(0, 0, 0)  # Preto
        pdf.multi_cell(0, 6, sanitize_text("Quando permitido pelas regras locais, um jogador pode obter alívio de uma condição anormal do campo:"), 0, "L")
        
        # Lista de regras
        pdf.ln(2)
        pdf.cell(10, 6, "", 0, 0)
        pdf.cell(0, 6, sanitize_text("• O jogador pode dropar a bola dentro do comprimento de um taco da posição original"), 0, 1)
        pdf.cell(10, 6, "", 0, 0)
        pdf.cell(0, 6, sanitize_text("• A nova posição não pode estar mais próxima do buraco"), 0, 1)
        pdf.cell(10, 6, "", 0, 0)
        pdf.cell(0, 6, sanitize_text("• A bola deve ser dropada de altura do joelho"), 0, 1)
        pdf.cell(10, 6, "", 0, 0)
        pdf.cell(0, 6, sanitize_text("• O alívio não é permitido em áreas de penalidade ou bunkers, exceto quando especificado"), 0, 1)
        pdf.ln(2)
        
        pdf.multi_cell(0, 6, sanitize_text("Esta regra ajuda a manter o jogo justo quando a bola está em condições desfavoráveis. O jogador deve anunciar sua intenção de utilizar esta regra antes de mover a bola, e todos os membros do grupo devem concordar com a aplicação do alívio na situação específica."), 0, "L")
        
        # Rodapé com logo Karam Design
        pdf.ln(10)
        pdf.set_font("helvetica", "", 10)
        pdf.set_text_color(100, 100, 100)
        pdf.cell(0, 6, sanitize_text("Design by:"), 0, 1, "C")
        pdf.image(karam_logo_path, x=pdf.w/2 - 15, y=pdf.get_y(), w=30)
        pdf.ln(20) # Aumentado de 15 para 20 para dar mais espaço após o logo
        pdf.cell(0, 6, sanitize_text("© 2025 Todos os direitos reservados a Karam Design"), 0, 1, "C")
